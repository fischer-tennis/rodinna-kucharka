...
async function uploadExtraImages(recipeId,files){if(!files?.length)return;const msg=$('detailMessage');msg.textContent=`Nahrávam ${files.length} fotografií…`;try{for(const file of files)await uploadGalleryImage(file,recipeId,'gallery');msg.textContent=`✓ Pridané fotografie: ${files.length}.`;await loadCloudData();openRecipe(recipeId)}catch(e){msg.textContent='Chyba: '+e.message}}
